...
import {CompositeDisposable, Emitter, Point, Range} from 'atom';
...
import {ATOMPSQL_ERRORS_FILE, AtomPsqlWatch} from './atom-psql-watch.js';
...
export default class AtomPsqlController {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'atom-psql:send_rollback': () => {
        if (!this.serviceInitFlag) {
          atom.notifications.addWarning("psql is not ready action abroted");
          this.initServices();
          return;
        }
      ...
      'atom-psql:transaction_state': () => {
        let state = transactionManager.getState();
        atom.notifications.addInfo('TRANSACTION STATE:' + transactionManager.stateToString(state));
      },
      'atom-psql:show_terminal_erors':()=>{
        if (fs.existsSync(ATOMPSQL_ERRORS_FILE)) {
          atom.workspace.open(ATOMPSQL_ERRORS_FILE);
        } else {
          atom.notifications.addWarning('canot find atom psql erorrs log');
        }
      },
    }));
  ...
  exec_selection_sql(rtype, query, setting_echo_on) {
    ...
    if (rtype == 'term') {
      ...
      if (queries.length == 1) {
        let cmd = queries[0];
        psqlCommandControllerInstance.checkCommand(cmd);
        ...
        let intervalId = setInterval(() => {
          try {
            let q = queries.shift();
            if (q) {
              psqlCommandControllerInstance.checkCommand(q);
    ...
    psql.exec(query, (error, stdout, stderr) => {
      ...
      let showPsqlResultHtml = function () {
        let htmlView = new PsqlHtmlView({'ordinal': ordinal, 'title': title, 'filePath': file_new, 'fileType': rtype});
