...
//	terminalViews = [];
  ...
  _initVariablesView() {
    ...
    if (!this.variablesView) {
     // console.log("VARIABLES NOT EXIST ");
      ...
      //console.log("VARIABLES NOT LIVE");
      initFlag = true;
      //se periptosi pou exi afere8i apo to xristi to item apo to pane
      this.variablesView.close();
      // let pane = atom.workspace.paneForItem(this.variablesView);
      // if (pane) {
      //   pane.destroyItem(this.variablesView);
      // }
    }
    if (initFlag) {
  ...
  toggle() {
    //console.log("terminal-controller toggle");
  ...
  grabLine(y) {
    this.psqlTerminalView.grabLine(y);
  }
  ...
  _createPsql(PGPASS) {
    ...
    // let onExit = function () {
    //   console.log("PSQL onEXIT");
    //   if (self.variablesView) {
    //     console.log("PSQL onEXIT DESTROY VARIABLE VIEW");
    //     let pane = atom.workspace.paneForItem(self.variablesView);
    //     //console.log("REMOVE PANE1",pane);
    //     if (pane) {
    //       //console.log("REMOVE VARIABLES PANE ITEM");
    //       pane.destroyItem(self.variablesView);
    //      } else {
    //        self.variablesView.destroy();
    //      }
    //     //console.log("VARIABLES SET TO NULL");
    //     self.variablesView = null;
    //   }
    // };
    let onKey = function (data) {
      if (data.charCodeAt(0) == 13) {
        ...
        if (m = line.match(self.regexp_prompt1)) {
          ...
          let len = m[0].length;
          let cmd1 = line.substring(len, x).trim();
